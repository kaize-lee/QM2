...
#creating choropleth for 

m_age = fm.Map(location=[51.5074, 0], zoom_start=9.5,tiles=None)
fm.TileLayer('CartoDB positron',name="Light Map",control=False).add_to(m_age)

#age dataset
data_path = "./data/Age per Borough.csv"
age = pd.read_csv(data_path, encoding = 'latin1')

#merging the age dataset on the shapefile dataset
geoage = pd.merge(boroughGeodataNoCity, age, left_on='code', right_on ='Code')
geoage.head()
...
interactive_tooltip_age = fm.features.GeoJson(
    geoage,
    style_function = style_function, 
    control = False,
    highlight_function = highlight_function, 
    tooltip = fm.features.GeoJsonTooltip(
        fields = ["name","Average age (2016)"],
        aliases = ["Borough: ","Average age"],
        style = ("background-color: white; color: #; font-family: arial; font-size: 12px;") 
    )
)
m_age.add_child(interactive_tooltip_age)
m_age.keep_in_front(interactive_tooltip_age)
fm.LayerControl().add_to(m_age)
m_age
